...
- **小さい文字は使わない。** 円形画面では読みにくいので、ヒント文字は置かず、
  操作は山形と点で示す
...
| 色 | ダンス | 名前 | 絵 |
| --- | --- | --- | --- |
| 🟢 `0x22C55E` | `simple_nod` | うんうん | 縦の両矢印 ↕ |
| 🔵 `0x3B82F6` | `side_to_side_sway` | ゆらゆら | 横の両矢印 ↔ |
| 🟡 `0xF59E0B` | `dizzy_spin` | くるくる | 丸い矢印 ↻ |
| 🔴 `0xEF4444` | `chicken_peck` | つんつん | 点をつつく斜めの矢印 |

### 文字の大きさについて

**M5GFX の日本語フォントは `lgfxJapanGothic_40` が最大。** `setTextSize()` で拡大すると
ビットマップフォントなのでギザギザになるため、等倍で使う。大きくしたいときは
フォントを増やすしかない。

絵を消してひらがなだけにする見え方も試せる（`ui::State::showIcon` を false、
プレビューでは `I` キー）。今は絵ありを採用している。
...
`./tools/preview/build.sh` で、実機と同じ描画コードを Mac のウィンドウで動かせる。
`--shot out/` を付けると全画面を PNG に書き出して終わるので、目で見て詰められる。

そのために描画は `src/ui.cpp` と `src/icons.cpp` に分け、M5Unified・Wi-Fi・HTTP に
依存させていない（M5Unified に SDL 対応が無いため）。状態は `ui::State` で渡す。

この仕組みを入れたことで、書き込まずに次の不具合を見つけて直せた。

- **`drawArc` は輪郭線しか描かない。** 丸い矢印・くるくる・禁止マークがすべて
  細い線で描かれていた。太い帯にするには `fillArc`
- 丸い矢印の矢羽の向きが逆で、再生ボタン（▶）に見えていた。時計回りの接線は
  角度 `+90` の向き
- 「つながらない」画面の英字が円の縁に近すぎた
- 再生中のくるくるが太すぎ、下地が無くて動きが分かりにくかった
